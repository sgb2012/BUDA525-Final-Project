...
highSchoolGPA <- rescale(highSchoolGPA, to = c(0,100))
prevColGPA <- rescale(prevColGPA, to = c(0,100))
highestACT <- rescale(highestACT, to = c(0,100))
totalSAT <- rescale(totalSAT, to = c(0,100))
scoreGED <- rescale(scoreGED, to = c(0,100))
...
#Model Development
#Focus on comparing variables and manipulated data to "totalAwardWVU"

plot(highSchoolGPA~totalAwardWVU, data$finDataTableWVU, main = "High School GPA vs Total WVU Award", xlab = "High School GPA", ylab = "Total Award by WVU")
model.1 <- lm(highSchoolGPA~totalAwardWVU)
summary(model.1)
abline(model.1, col = "blue")

plot(prevColGPA~totalAwardWVU, data$finDataTableWVU, main = "Previous College GPA vs Total WVU Award", xlab = "Previous College GPA", ylab = "Total Award by WVU")
model.2 <- lm(prevColGPA~totalAwardWVU)
summary(model.2)
abline(model.2, col = "blue")

plot(highestACT~totalAwardWVU, data$finDataTableWVU, main = "Highest ACT Score vs Total WVU Award", xlab = "Highest ACT Score", ylab = "Total Award by WVU")
model.3 <- lm(highestACT~totalAwardWVU)
summary(model.3)
abline(model.3, col = "blue")

plot(totalSAT~totalAwardWVU, data$finDataTableWVU, main = "Total SAT Score vs Total WVU Award", xlab = "Total SAT Score", ylab = "Total Award by WVU")
model.4 <- lm(totalSAT~totalAwardWVU)
summary(model.4)
abline(model.4, col = "blue")

plot(scoreGED~totalAwardWVU, data$finDataTableWVU, main = "GED Score vs Total WVU Award", xlab = "GED Score", ylab = "Total Award by WVU")
model.5 <- lm(scoreGED~totalAwardWVU)
summary(model.5)
abline(model.5, col = "blue")

plot(scaledAcademAchiev~totalAwardWVU, data$finDataTableWVU, main = "Scaled Academic Achievement vs Total WVU Award", xlab = "Scaled Academic Achievement", ylab = "Total Award by WVU")
model.6 <- lm(scaledAcademAchiev~totalAwardWVU)
summary(model.6)
abline(model.6, col = "blue")

Model_A<-plot(expectedFamContrib~totalAwardWVU,data=finDataTableWVU)

Model_B<-plot(scaledAcademAchiev~totalAwardWVU)

Model_C<-plot(highSchoolGPA~totalAwardWVU)

Model1<-lm(totalSAT~totalAwardnonWVU)
summary(Model1)
plot(Model1)
Model1.2<-plot(totalSAT~totalAwardnonWVU)

Model2<-lm(highSchoolGPA~totalAwardnonWVU)
summary(Model2)
plot(Model2)
Model2.2<-plot(highSchoolGPA~totalAwardnonWVU)

Model3<-lm(scaledAcademAchiev~totalAwardnonWVU)
summary(Model3)
plot(Model3)
Model3.2<-plot(scaledAcademAchiev~totalAwardnonWVU)
