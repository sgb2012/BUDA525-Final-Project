...
tempStudentEthn <- firstTimeFresh$ETHNIC.CODE
summary(tempStudentEthn)
tempStudentEthn[is.na(tempStudentEthn)] = 7
studentEthn <- as.factor(tempStudentEthn)
studentEthn <- factor(tempStudentEthn, labels = c("White", "Black", "Hispanic", "Asian", "Native", "Unknown", "EMPTY"))
summary(studentEthn)
levels(studentEthn)
...
affinityStatus <- factor(affinityStatus, labels = c("No Affinity", "Some Affinity"))
summary(affinityStatus)

#Affordability attributes
expectedFamContrib <- rescale(firstTimeFresh$EXPECTED.TOT.FAM.CONT..EFC, to = c(1,100))
summary(expectedFamContrib)
expectedFamContribF <- cut(expectedFamContrib, breaks = c(1, 19.8, 39.6, 59.4, 79.2, 100), right = FALSE, include.lowest=TRUE)
expectedFamContribF <- factor(expectedFamContribF, labels = c("Very Low", "Low", "Medium","High", "Very High"))
levels(expectedFamContribF)
summary(expectedFamContribF)
...
expectedFamContribF -> finDataTableWVU$expectedFamContribF
...
#Add these subsets of students to our table
...
finDataTableWVU$highSchoolGPA[is.na(finDataTableWVU$highSchoolGPA)] = 0
finDataTableWVU$prevColGPA[is.na(finDataTableWVU$prevColGPA)] = finDataTableWVU$highestACT[is.na(finDataTableWVU$highestACT)] = 0
finDataTableWVU$totalSAT[is.na(finDataTableWVU$totalSAT)] = 0
finDataTableWVU$scoreGED[is.na(finDataTableWVU$scoreGED)] = 0
finDataTableWVU$expectedFamContrib[is.na(finDataTableWVU$expectedFamContrib)] = 0
finDataTableWVU$scaledAcademAchiev[is.na(finDataTableWVU$scaledAcademAchiev)] = 0
...
#Assess and compare the predictive abilities of both models
#To use 5 fold cross validation, we'll split the data into 5 subsets.

dim(finDataTableWVU)
finDataTableWVU$SampID = rep(5, dim(finDataTableWVU)[1])
8590/5

set.seed(1)
set = sample(1:dim(finDataTableWVU)[1])  
for(j in 1:4){
  finDataTableWVU$SampID[set[((j-1)*1718+1):(j*1718)]]=j  
}

RSS1 = 0
RSS3 = 0
for(i in 1:5){
  sets = which(finDataTableWVU$SampID == i)
  
  #fit models with sets removed
  model1 <- lm(sqrt(totalAwardWVU) ~ studentGender + studentEthn + degreeF + proxStudentType + affinityStatus + expectedFamContrib + expectedFamContrib.NA + aidStudentnonWVU + aidStudentWVU + aidStudentPromScholar + affinityStatus + highestACT + highestACT.NA + totalSAT + totalSAT.NA + totalAwardnonWVU + totalAwardPromScholar, data = finDataTableWVU[-sets,])
  
  model3 <- lm(sqrt(totalAwardWVU) ~ studentGender + studentEthn + degreeF + degreeType + studentLoc + stateIndicator + proxStudentType + scaledAcademAchiev + affinityStatus + expectedFamContrib, data = finDataTableWVU[-sets,])
  
  #calculate predictions on the sets that were removed
  predModel1 <-predict(model1, newdata = finDataTableWVU[sets,])
  predModel3 <-predict(model3, newdata = finDataTableWVU[sets,])
 
  #Calculate the residual sum of squares 
  RSS1 = RSS1 + sum((sqrt(finDataTableWVU$totalAwardWVU[sets]) - predModel1)^2)
  RSS3 = RSS3 + sum((sqrt(finDataTableWVU$totalAwardWVU[sets]) - predModel3)^2)
}
RSS1
RSS3
